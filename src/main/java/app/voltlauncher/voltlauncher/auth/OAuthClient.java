...
public final class OAuthClient {
    ...
    public String buildAuthUrl(String state, String codeVerifier) {
        return AUTHORIZE_ENDPOINT + "?" + String.join("&", param("client_id", clientId), param("response_type", "code"), param("redirect_uri", REDIRECT_URI), param("scope", SCOPE), param("prompt", "select_account"), param("state", state), param("code_challenge", codeChallenge(codeVerifier)), param("code_challenge_method", "S256"));
    }
    ...
    private TokenResponse parseTokenResponse(JSONObject json, String fallbackRefresh) {
        ...
        String stored = !newRefresh.isBlank() ? newRefresh : (fallbackRefresh != null ? fallbackRefresh : "");
        return new TokenResponse( json.getString("access_token"), stored, System.currentTimeMillis() + TimeUnit.SECONDS.toMillis(expiresIn));
    }
