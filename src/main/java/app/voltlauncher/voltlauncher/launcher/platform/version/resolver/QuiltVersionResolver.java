...
public final class QuiltVersionResolver extends AbstractDelegatingPlatformResolver {

    private static final String QUILT_LOADERS_URL = "https://meta.quiltmc.org/v3/versions/loader/";
    ...
    public QuiltVersionResolver(IVersionResolver vanillaResolver) {
        super(PlatformRegistry.QUILT_ID, vanillaResolver);
    }
    ...
    public List<AvailableVersion> listLoaderVersions(String minecraftVersionId) throws Exception {
        ...
        JSONArray loaders = http.getJsonArray(QUILT_LOADERS_URL + URLEncoder.encode(base, StandardCharsets.UTF_8));
    ...
    public JSONObject resolveMetadata(String versionId) throws Exception {
        ...
        if (loaderVersion.isBlank()) {
            List<AvailableVersion> loaders = listLoaderVersions("quilt:" + selection.minecraftVersion());
            if (loaders.isEmpty()) {
                throw new IllegalStateException("No Quilt loader found for Minecraft " + selection.minecraftVersion());
            }
        ...
        base.put("id", "quilt:" + selection.minecraftVersion() + ":" + loaderVersion);
    ...
    private String extractMinecraftVersion(String versionId) {
        ...
        if (raw.startsWith("quilt:")) {
            String suffix = raw.substring("quilt:".length());
