...
<template>
  <div class="flex-1 flex-col overflow-hidden" :class="activeTab === 'profiles' ? 'flex' : 'hidden'" @click="closeMenu">
    ...
    <template v-if="!managingInstance">
      ...
      <div class="grid grid-cols-[repeat(auto-fit,minmax(250px,1fr))] gap-3 px-4 py-3 md:px-6">
        <!-- New profile card -->
        <button
          type="button"
        ...
        <div
          v-for="pending in pendingInstances"
          :key="pending.id"
        ...
        >
          <!-- Image area -->
          <div class="relative flex h-[100px] items-center justify-center overflow-hidden rounded-t-xl bg-[linear-gradient(135deg,#0a1535,#122050)]">
          ...
          <div class="px-3 py-2.5">
            <div class="mb-1 truncate text-[length:var(--text-md-plus)] font-semibold text-white">{{ pending.name }}</div>
        ...
        >
          <!-- Card image area -->
          <div class="relative flex h-[100px] items-center justify-center overflow-hidden rounded-t-xl pointer-events-none">
            <div
            ...
            <div class="pointer-events-auto absolute inset-0 flex items-center justify-center bg-[var(--accent-overlay)] opacity-0 transition-opacity duration-200 group-hover:opacity-100">
              <button
                type="button"
                class="inline-flex items-center gap-2 rounded-[7px] bg-[var(--primary)] px-4 py-2 text-[length:var(--text-base)] font-bold tracking-[0.08em] text-white disabled:opacity-50 disabled:cursor-not-allowed"
                :disabled="!authData || isLaunching"
                @click.stop="playInstance(inst)"
              >
          ...
          <div class="px-3 py-2.5 pointer-events-none">
            ...
            <div class="flex flex-wrap gap-[5px] text-[length:var(--text-2xs)] text-white/60">
              <span class="inline-flex items-center gap-1"><span class="size-1.5 rounded-full bg-[#4caf50]"></span>{{ formatLoaderId(inst.versionId) }}</span>
          ...
          <div class="absolute right-[7px] top-[7px] z-10" @click.stop>
            ...
            >
              <button type="button" class="flex w-full items-center gap-2.5 px-3 py-2 text-[length:var(--text-sm)] text-white/70 transition-colors hover:bg-white/8 hover:text-white" @click="startEdit(inst)">
                <Icon icon="lucide:pencil" class="size-3.5 shrink-0" />Umbenennen
              </button>
              <button type="button" class="flex w-full items-center gap-2.5 px-3 py-2 text-[length:var(--text-sm)] text-white/70 transition-colors hover:bg-white/8 hover:text-white" @click="openFolder(inst)">
                <Icon icon="lucide:folder-open" class="size-3.5 shrink-0" />Ordner öffnen
              </button>
              <div class="mx-2 my-0.5 h-px bg-white/7"></div>
              <button type="button" class="flex w-full items-center gap-2.5 px-3 py-2 text-[length:var(--text-sm)] text-[var(--danger-text,#f87171)] transition-colors hover:bg-red-500/10" @click="startDelete(inst)">
                <Icon icon="lucide:trash-2" class="size-3.5 shrink-0" />Löschen
              </button>
            </div>
          </div>
        </div>
    ...
    <template v-else>
      <div class="flex flex-1 flex-col overflow-hidden px-4 pt-4 md:px-6" @click.stop>
        ...
        <div class="mb-5 flex items-center gap-3">
          ...
          <div class="text-[length:var(--text-lg)] font-bold tracking-[0.1em] text-white">{{ managingInstance.name.toUpperCase() }}</div>
          <span v-if="managingInstance.running" class="rounded border border-[var(--success-border)] bg-[var(--success-bg)] px-2 py-0.5 text-[10px] font-bold tracking-[0.08em] text-[var(--accent)]">▶ AKTIV</span>
        </div>

        <div class="flex flex-1 gap-5 overflow-hidden">
          <!-- Left: hero card -->
          <div class="relative flex w-56 shrink-0 flex-col overflow-hidden rounded-xl border border-white/10 bg-[var(--surface-panel)]">
            <div
              class="flex h-44 items-center justify-center text-[60px]"
              :class="versionGradientClass(managingInstance.versionType)"
            >
              {{ versionEmoji(managingInstance.versionType) }}
            </div>
            <div class="flex flex-col gap-1.5 p-3">
              <div class="truncate text-[length:var(--text-md)] font-semibold text-white">{{ managingInstance.name }}</div>
              <div class="text-[length:var(--text-xs)] text-white/50">{{ formatLoaderId(managingInstance.versionId) }}</div>
            </div>
            <!-- Play button -->
            <div class="px-3 pb-3">
              <button
                type="button"
                class="w-full inline-flex items-center justify-center gap-2 rounded-[7px] border border-[var(--accent-border-strong)] bg-[var(--primary)] py-2 text-[length:var(--text-sm)] font-bold tracking-[0.08em] text-white transition-all hover:shadow-[var(--shadow-accent-md)] disabled:opacity-50 disabled:cursor-not-allowed"
                :disabled="!authData || isLaunching || managingInstance.running"
                @click="playInstance(managingInstance)"
              >
                <Icon icon="lucide:play" class="size-[10px]" />SPIELEN
              </button>
            </div>
          </div>
          ...
          <div class="flex flex-1 flex-col gap-4 overflow-y-auto">
            <!-- Stats -->
            <div class="rounded-xl border border-white/8 bg-[var(--surface-panel)] p-4">
              ...
              <div class="grid grid-cols-2 gap-3">
                <div class="flex flex-col gap-0.5">
                  ...
                  <div class="text-[length:var(--text-sm)] font-medium text-white">{{ formatLoaderId(managingInstance.versionId) }}</div>
                </div>
                <div class="flex flex-col gap-0.5">
            ...
            <div class="rounded-xl border border-white/8 bg-[var(--surface-panel)] p-4">
              ...
              <div class="flex flex-col gap-1.5">
                ...
                <button
                  ...
                  :disabled="managingInstance.running"
                  @click="startDelete(managingInstance)"
                >
  ...
  <Teleport to="body">
    <div
      v-if="showEditModal && editTargetInstance"
      class="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 p-5 backdrop-blur-md"
      @click.self="showEditModal = false"
    >
      <div class="w-full max-w-[420px] rounded-[14px] border border-[var(--accent-border)] bg-[var(--surface-panel-strong)] shadow-[var(--shadow-modal)]">
        <div class="flex items-center justify-between border-b border-white/7 px-5 py-4">
          ...
          <button type="button" class="flex size-7 items-center justify-center rounded-md border border-white/10 bg-white/5 text-white/40 transition-all duration-200 hover:bg-white/10 hover:text-white" @click="showEditModal = false">
        ...
        <div class="flex flex-col gap-4 p-5">
          ...
          <div class="flex justify-end gap-2">
            <button type="button" class="inline-flex items-center gap-1.5 rounded-[7px] border border-white/10 bg-white/5 px-3.5 py-[7px] text-[length:var(--text-sm)] font-semibold tracking-[0.07em] text-white/45 transition-all duration-200 hover:bg-white/10" @click="showEditModal = false">
              Abbrechen
            </button>
            <button
              type="button"
  ...
  <Teleport to="body">
    <div
      v-if="showDeleteModal && deleteTargetInstance"
      class="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 p-5 backdrop-blur-md"
      @click.self="showDeleteModal = false"
    >
      <div class="w-full max-w-[400px] rounded-[14px] border border-white/10 bg-[var(--surface-panel-strong)] shadow-[var(--shadow-modal)]">
        <div class="flex items-center justify-between border-b border-white/7 px-5 py-4">
          <div class="text-[length:var(--text-lg)] font-bold tracking-[0.1em] text-white">PROFIL LÖSCHEN</div>
          <button type="button" class="flex size-7 items-center justify-center rounded-md border border-white/10 bg-white/5 text-white/40 transition-all duration-200 hover:bg-white/10 hover:text-white" @click="showDeleteModal = false">
            <Icon icon="lucide:x" class="size-[14px]" />
          </button>
        </div>
        <div class="flex flex-col gap-4 p-5">
          <p class="text-[length:var(--text-base)] text-white/70">
            Profil <span class="font-semibold text-white">{{ deleteTargetInstance.name }}</span> wirklich löschen? Diese Aktion kann nicht rückgängig gemacht werden.
          </p>
          <div class="flex justify-end gap-2">
            <button type="button" class="inline-flex items-center gap-1.5 rounded-[7px] border border-white/10 bg-white/5 px-3.5 py-[7px] text-[length:var(--text-sm)] font-semibold tracking-[0.07em] text-white/45 transition-all duration-200 hover:bg-white/10" @click="showDeleteModal = false">
              Abbrechen
            </button>
            <button type="button" class="inline-flex items-center gap-1.5 rounded-[7px] border border-red-500/40 bg-red-500/20 px-3.5 py-[7px] text-[length:var(--text-sm)] font-semibold tracking-[0.07em] text-red-400 transition-all duration-200 hover:bg-red-500/30" @click="handleDeleteInstance(deleteTargetInstance!.name)">
